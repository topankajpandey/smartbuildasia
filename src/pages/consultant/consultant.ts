...
export class ConsultantPage {
  ...
  //public submission = {photos_url:'', description:'', company_name:'', company_role:'', contr_name:'', contr_phone:'', contr_email:''};
  ...
  ngAfterViewInit() {
    ...
    this.authService.get_submission_data(submission_data).then((result) => {
      ...
      if(this.data && this.data.status_code==200){
          ...
          //this.consultantText = this.data.response.detail;
          //this.consultant = 

            if(this.action=='update_submission'){
              this.consultant = { 
  ...
  text_card_submission(){
    ...
    this.consultantText['project_id'] = localStorage.project_id;
    ...
    this.cameraService.project_submission(this.consultantText).then((result) =>
    {
        ...
        this.data = result;
        if(this.data && this.data.status_code==200){
          this.alertForLocator('Details submitted successfully.', 'Success', 'Ok');

          if(this.submit_type=='via_submission_edit'){
  ...
  bussiness_card_submission(){
    ...
    this.cameraService.project_submission(this.consultant).then((result) => {
        ...
        this.data = result;
        if(this.data && this.data.status_code==200){
          this.alertForLocator('Business card added successfully.', 'Success', 'Ok');
          if(this.submit_type=='via_submission_edit'){
            this.navCtrl.push(SubmissionPage)
            .then(() => {
              this.navCtrl.setRoot(SubmissionPage);
